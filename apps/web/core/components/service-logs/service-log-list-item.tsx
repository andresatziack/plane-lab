            ...
            {batch.segments.map((segment) => (
              <div key={segment.id} className="text-sm flex flex-wrap items-center gap-1.5">
                ...
                {segment.is_hour_type_overridden ? (
                  <Tooltip
                    tooltipContent={`${t("work_item.service_log.badges.suggested_classification", { name: segment.suggested_hour_type_name })}${segment.classification_reason ? `. ${segment.classification_reason}` : ""}`}
                    position="top"
                  >
                    <Info className="text-custom-text-400 size-3" />
                  </Tooltip>
                ) : (
