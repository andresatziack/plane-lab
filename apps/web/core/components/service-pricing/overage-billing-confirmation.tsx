    ...
    <ModalCore isOpen={isOpen} handleClose={onClose}>
      <div className="flex flex-col gap-4 p-5">
        <div className="flex items-start gap-3">
          <span className="grid size-10 shrink-0 place-items-center rounded-full bg-red-500/15">
            <AlertTriangle className="size-5 text-red-500" />
          </span>
          <div className="flex flex-col gap-1">
            <h3 className="text-lg font-medium text-custom-text-100">
              {t("work_item.service_overage.title")}
            </h3>
        ...
        {errorCode === "OVERAGE_RATE_NOT_CONFIGURED" && (
          <div className="rounded-md border border-red-500/40 bg-red-500/5 px-3 py-2">
            <p className="text-sm text-custom-text-200">
              {t("work_item.service_overage.no_rate")}
            </p>
          </div>
        )}
        ...
        {preview && (
          <div className="flex flex-col gap-2 rounded-md border border-custom-border-200 p-3">
            <div className="flex items-center justify-between">
              ...
              <span className="text-sm text-custom-text-100">{preview.overage_hours}</span>
            </div>
            <div className="flex items-center justify-between">
              ...
              <span className="text-sm text-custom-text-100">
                ...
                <span className="ml-1 text-xs text-custom-text-350">
                  ({t(`work_item.service_overage.rate_source.${preview.rate_source}`)})
                </span>
              </span>
            </div>
            <div className="flex items-center justify-between border-t border-custom-border-200 pt-2">
              <span className="text-sm font-medium text-custom-text-100">
                {t("work_item.service_overage.amount")}
              </span>
              <span className="text-base font-semibold text-custom-text-100">R$ {preview.amount}</span>
            </div>
          </div>
        )}
        ...
        {preview && !preview.is_reversible && (
          <p className="text-sm font-medium text-red-500">{t("work_item.service_overage.irreversible")}</p>
        )}
